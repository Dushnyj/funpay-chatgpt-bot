...
До обновления обязательно сделать `pg_dump` и архив `.env`/конфигурации nginx. Alembic запускается автоматически до старта lifecycle; при неполной legacy-схеме приложение останавливается вместо небезопасного stamp. Миграция сроков переводит `Duration.days` в минуты с сохранением ID и внешних ключей, удаляет недостоверные Chat-поля, нормализует исторические настройки capacity и добавляет durable state для replacement/expiry. Исторические terminal-аренды помечаются уже уведомлёнными, поэтому старые сообщения после обновления не рассылаются. Поэтому **любой rollback с ревизии `20260713_0015` выполняется только восстановлением pre-upgrade `pg_dump`**: `alembic downgrade` возвращает форму схемы, но не является восстановлением утраченных исходных данных.
