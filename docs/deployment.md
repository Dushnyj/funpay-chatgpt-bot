...
До обновления обязательно сделать `pg_dump` и архив `.env`/конфигурации nginx. Alembic запускается автоматически до старта lifecycle; при неполной legacy-схеме приложение останавливается вместо небезопасного stamp. Миграция сроков переводит `Duration.days` в минуты с сохранением ID и внешних ключей, нормализует исторические настройки capacity и добавляет durable state для replacement/expiry. Ревизия `20260713_0016` отделяет подтверждённые продажи от произвольных личных чатов: старые диалоги не удаляются, но остаются скрытыми, пока их покупатель и chat node не подтверждены через `get_sales()` или сохранённый sale-order. Ревизия `20260714_0017` сохраняет cursor исторического backfill: один цикл получает актуальную head-страницу и не более одной следующей страницы, после завершения истории повторно сканируется только head. Исторические terminal-аренды помечаются уже уведомлёнными, поэтому старые сообщения после обновления не рассылаются. Поэтому **любой rollback с ревизий `20260713_0015`–`20260714_0017` выполняется только восстановлением pre-upgrade `pg_dump`**: `alembic downgrade` возвращает форму схемы, но не восстанавливает потерянное или переинтерпретированное состояние.
