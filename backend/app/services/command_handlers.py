...
async def _lock_resolved_rental(
    ...
    if order is None or not await is_verified_bot_sale_order(session, order):
        return None
    rental = (
        await session.execute(
            select(Rental)
