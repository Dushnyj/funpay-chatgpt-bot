...
- **Проверка через браузер**: Device Auth подтверждает аккаунт и получает токены в обычном браузере; если headless-проверку пароля/TOTP останавливает Cloudflare, панель предлагает ограниченное по времени ручное подтверждение вместо ложной общей ошибки
